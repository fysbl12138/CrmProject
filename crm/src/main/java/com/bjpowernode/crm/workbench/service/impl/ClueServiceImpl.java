...
public class ClueServiceImpl implements ClueService {
    ...
    @Override
    public PaginationVO<Clue> selectPageListByElenment(Map<String, Object> map) {

        int total = clueDao.getTotal();

        List<Clue> dataList = clueDao.selectPageList(map);
    ...
    @Override
    public boolean deleteActivityAndClueById(String id) {
        boolean flag = true;
        int count = clueActivityRelationDao.deleteActivityAndClueById(id);
    ...
    @Override
    public boolean bundActivityAndClue(String cid, String[] aids) {
        ...
        for (String aid: aids) {
            ...
            int count = clueActivityRelationDao.bundActivityAndClue(clueActivityRelation);
            if(count != 1){
                flag = false;
            }
        }
    ...
    @Override
    public List<Activity> getClueActivity(String aname) {

        List<Activity> list = activityDao.getActivityByName(aname);
...
//        (2) 通过线索对象提取客户信息，当该客户不存在的时候，新建客户（根据公司的名称精确匹配，判断该客户是否存在！）
        ...
        Customer cus = customerDao.getCustomerByName(companyName);
...
//        (10) 删除线索
        int count10 = clueDao.detele(clueId);
        if(count10 != 1){
            flag = false;
        }
